...
| `audio-dataset-curation` | Audio Dataset Curation | 3 optional | Collect, clean, annotate, and DVC-track production audio clips for STT experiments. |
...
| `post-correction-experiment` | Post-Correction Experiment | 3 optional | Evaluate a specific post-correction strategy on ASR transcripts for entity accuracy gain vs latency cost. |
| `stt-benchmark-run` | STT Benchmark Run | 4 optional | End-to-end evaluation of one STT configuration on gold-92: transcription, metric computation, and significance testing. |
| `whisper-finetuning-run` | Whisper Fine-Tuning Run | 4 optional | Train a domain-adapted Whisper checkpoint on Rezolve production audio and evaluate on gold-92. |
...
<details>
<summary><strong>Audio Dataset Curation</strong>
(<code>audio-dataset-curation</code>)</summary>

| Field | Value |
|---|---|
| **ID** | `audio-dataset-curation` |
| **Description** | Collect, clean, annotate, and DVC-track production audio clips for STT experiments. |
| **Optional steps** | `research-internet`, `research-code`, `planning` |

Covers tasks that source audio from Rezolve production logs or the rail-artifacts voice
samples repository, apply quality filtering, annotate ground-truth transcripts and entity
spans, and version the result with DVC against the Azure Blob remote. Unlike download-dataset
(which fetches public, pre-labeled sets), this type involves human or LLM-assisted labelling
of proprietary audio. Gold-92 curation and any future production audio expansion tasks belong
here.
...
<details>
<summary><strong>Post-Correction Experiment</strong>
(<code>post-correction-experiment</code>)</summary>

| Field | Value |
|---|---|
| **ID** | `post-correction-experiment` |
| **Description** | Evaluate a specific post-correction strategy on ASR transcripts for entity accuracy gain vs latency cost. |
| **Optional steps** | `planning`, `research-code`, `compare-literature` |

Covers tasks that take raw ASR transcripts from a prior stt-benchmark-run and apply a
correction strategy — LLM correction pass, custom vocabulary injection, contextual entity
boosting, or a combination. The task measures the delta in entity accuracy and action-critical
WER on gold-92 compared to the uncorrected transcript, plus the added per-utterance latency.
Results must include a cost estimate (LLM tokens or API calls) and should be compared against
at least one other correction strategy.

</details>

<details>
<summary><strong>STT Benchmark Run</strong> (<code>stt-benchmark-run</code>)</summary>

| Field | Value |
|---|---|
| **ID** | `stt-benchmark-run` |
| **Description** | End-to-end evaluation of one STT configuration on gold-92: transcription, metric computation, and significance testing. |
| **Optional steps** | `planning`, `setup-machines`, `teardown`, `compare-literature` |

Covers tasks that run a fully specified STT configuration (model/API + optional
post-correction + optional routing policy) on the gold-92 benchmark and produce entity
accuracy, WER, intent preservation, and latency metrics with BCa bootstrap confidence
intervals. Every evaluation task in this project follows this shape: DVC-pull audio, call
transcription, compute metrics, run significance test vs Deepgram baseline, package results.
Unlike baseline-evaluation (which is open-ended), this type has a fixed output schema tied to
the registered project metrics.

</details>

<details>
<summary><strong>Whisper Fine-Tuning Run</strong>
(<code>whisper-finetuning-run</code>)</summary>

| Field | Value |
|---|---|
| **ID** | `whisper-finetuning-run` |
| **Description** | Train a domain-adapted Whisper checkpoint on Rezolve production audio and evaluate on gold-92. |
| **Optional steps** | `planning`, `research-code`, `setup-machines`, `teardown` |

Covers tasks that prepare a training split from DVC-tracked production audio, run supervised
fine-tuning of a Whisper-class model (locally on Mac Metal via MPS or on an Azure H100 VM),
and evaluate the checkpoint on gold-92 for entity accuracy, WER regression, and latency. Tasks
must include a data-scale experiment or ablation so the minimum-viable dataset size can be
estimated. Gold-92 is held out strictly for evaluation — never included in training or
validation.
