...
These predictions capture the per-instance output of UsefulSensors Moonshine base (~60M parameters, ONNX CPU) on the gold-92
...
UsefulSensors Moonshine base (~60M parameters, ONNX CPU) run locally via `faster-whisper` (CTranslate2 INT8 quantization, CPU
