"""Run Whisper turbo with initial_prompt (vocabulary biasing) on all 93 gold-92 clips.

Saves outputs to data/whisper_turbo_biased_transcripts.json.
Uses faster-whisper with device='cpu', compute_type='int8', language='en',
and initial_prompt set to the 31-term domain vocabulary string.
"""
...
from tasks.t0004_vocabulary_biasing_experiment.code.constants import INITIAL_PROMPT
from tasks.t0004_vocabulary_biasing_experiment.code.load_dataset import (
    CYRILLIC_ANOMALY_CLIP,
    load_gold92,
)
from tasks.t0004_vocabulary_biasing_experiment.code.paths import WHISPER_TURBO_BIASED_TRANSCRIPTS
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run Whisper turbo with vocabulary biasing on gold-92"
    )
    parser.add_argument(
        "--limit",
        type=int,
...
def transcribe_clip(
    *,
    ...
    """Transcribe a single audio clip with Whisper turbo using vocabulary biasing.

    Returns dict with clip_id, hypothesis, latency_seconds.
    language='en' is mandatory to prevent accent-misclassification.
    initial_prompt provides domain vocabulary hints to guide transcription.
    """
...
def main() -> None:
    """Run Whisper turbo biased inference on gold-92 clips."""
    args = parse_args()

    print("Domain vocabulary initial_prompt:")
    print(f"  {INITIAL_PROMPT[:100]}...")
    print()
    print("Loading Whisper turbo model (device=cpu, compute_type=int8)...")
    ...
    if args.limit is None and len(clips) >= WARMUP_CLIPS:
        ...
        for warmup_clip in clips[:WARMUP_CLIPS]:
            if warmup_clip.audio_path.exists():
                _warmup_segments, _ = model.transcribe(
                    str(warmup_clip.audio_path),
                    language="en",
                    initial_prompt=INITIAL_PROMPT,
                    beam_size=5,
                )
    ...
    for i, clip in enumerate(tqdm(process_clips, desc="Whisper turbo biased inference")):
        ...
        try:
            ...
            if (i + 1) % 10 == 0:
                print(f"Whisper turbo biased: {i + 1}/{len(process_clips)} clips processed")

        except Exception as exc:  # noqa: BLE001
            warnings.warn(
                f"Whisper turbo transcription failed for {clip.clip_id}: {exc}", stacklevel=2
            )
            failed_count += 1
            results.append(
                {
    ...
    # Save results
    WHISPER_TURBO_BIASED_TRANSCRIPTS.parent.mkdir(parents=True, exist_ok=True)
    with WHISPER_TURBO_BIASED_TRANSCRIPTS.open("w", encoding="utf-8") as fh:
        json.dump(results, fh, indent=2, ensure_ascii=False)

    print(f"\nDone: {success_count}/{total} clips transcribed successfully.")
    print(f"Saved to {WHISPER_TURBO_BIASED_TRANSCRIPTS}")
    if failed_count > 0:
