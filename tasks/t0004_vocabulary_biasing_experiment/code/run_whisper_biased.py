"""Run Whisper Large v3 with initial_prompt (vocabulary biasing) on all 93 gold-92 clips.

Saves outputs to data/whisper_large_v3_biased_transcripts.json.
...
from tasks.t0004_vocabulary_biasing_experiment.code.paths import WHISPER_BIASED_TRANSCRIPTS
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run Whisper Large v3 with vocabulary biasing on gold-92"
    )
    parser.add_argument(
        "--limit",
        type=int,
...
def transcribe_clip(
    *,
    ...
    """Transcribe a single audio clip with Whisper Large v3 using vocabulary biasing.
...
def main() -> None:
    """Run Whisper Large v3 biased inference on gold-92 clips."""
    args = parse_args()
    ...
    for i, clip in enumerate(tqdm(process_clips, desc="Whisper Large v3 biased inference")):
        ...
        try:
            ...
            if (i + 1) % 10 == 0:
                print(f"Whisper Large v3 biased: {i + 1}/{len(process_clips)} clips processed")
    ...
    # Save results
    WHISPER_BIASED_TRANSCRIPTS.parent.mkdir(parents=True, exist_ok=True)
    with WHISPER_BIASED_TRANSCRIPTS.open("w", encoding="utf-8") as fh:
        json.dump(results, fh, indent=2, ensure_ascii=False)

    print(f"\nDone: {success_count}/{total} clips transcribed successfully.")
    print(f"Saved to {WHISPER_BIASED_TRANSCRIPTS}")
    if failed_count > 0:
