"""Compute all metrics for FunASR Paraformer variants and write results.

Compares vs:
  - Parakeet TDT 0.6b-v3 production baseline (t0009)
  - Granite Speech 4.1 2B biased (t0007)
"""
...
from tasks.t0010_funasr_paraformer_benchmark.code.constants import DOMAIN_VOCAB
from tasks.t0010_funasr_paraformer_benchmark.code.load_dataset import load_gold92
...
def main() -> None:
    ...
    for path, name in [
        (PARAFORMER_BATCH_TRANSCRIPTS, "paraformer batch"),
        (PARAFORMER_BIASED_TRANSCRIPTS, "paraformer biased"),
    ]:
        if not path.exists():
            raise RuntimeError(f"{name} transcripts not found at {path}. Run inference first.")

    batch_transcripts = load_transcripts(PARAFORMER_BATCH_TRANSCRIPTS)
    biased_transcripts = load_transcripts(PARAFORMER_BIASED_TRANSCRIPTS)

    systems: list[SystemMetrics] = [
        compute_system_metrics(
            variant_id="paraformer-large-batch",
            label="Paraformer-large — batch (no biasing)",
            clips=clips,
            transcripts=batch_transcripts,
            anomaly_clips=anomaly_clips,
        ),
        compute_system_metrics(
            variant_id="seaco-paraformer-large-biased",
            label="SeACo-Paraformer-large — contextual biased",
            clips=clips,
    ...
    for sys in systems:
        if sys.wer > 0.6:
            print(
                f"  WARNING: {sys.label} WER={sys.wer:.3f} > 0.6 "
                f"— model may not be suitable for this dataset."
            )
    ...
    batch_sys = next(s for s in systems if s.variant_id == "paraformer-large-batch")
    biased_sys = next(s for s in systems if s.variant_id == "seaco-paraformer-large-biased")
    biasing_gain = {
    ...
    metrics_out: dict[str, Any] = {"variants": variants, "biasing_gain": biasing_gain}
    ...
    transcripts_map = {
        "paraformer-large-batch": batch_transcripts,
        "seaco-paraformer-large-biased": biased_transcripts,
    }
    ...
    with ANALYSIS_OUTPUT.open("w", encoding="utf-8") as fh:
        json.dump(analysis_out, fh, indent=2, ensure_ascii=False)
    ...
    print("-" * 95)
    print(
        f"{'Parakeet TDT prod (t0009)':<45} "
        f"{parakeet_prod['ea']:>7.4f} {parakeet_prod['ea_dv']:>7.4f} "
        f"{parakeet_prod['wer']:>7.4f} {parakeet_prod['acwer']:>7.4f} "
        f"{parakeet_prod['ip']:>7.4f} {parakeet_prod['lat']:>8}"
    )
    print(
        f"{'Granite 4.1 2B biased (t0007)':<45} "
        f"{granite_biased['ea']:>7.4f} {granite_biased['ea_dv']:>7.4f} "
        f"{granite_biased['wer']:>7.4f} {granite_biased['acwer']:>7.4f} "
        f"{granite_biased['ip']:>7.4f} {granite_biased['lat']:>8}"
    )
    for sys in systems:
        marker = " ← baseline" if sys.variant_id == "paraformer-large-batch" else ""
        print(
            f"{sys.variant_id:<45} "
