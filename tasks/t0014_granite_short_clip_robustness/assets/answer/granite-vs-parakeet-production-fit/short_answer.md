...
Integration effort is low: reading brainpowa-realtime-api base.py confirms the STTAdapter base
class already implements transcribe_stream() as accumulate-then-transcribe, so a granite.py
adapter only needs to implement transcribe() and model loading — approximately 60–80 lines of
Python.
