"""Compute all metrics for Nemotron 3.5 ASR variants and write results.
...
from tasks.t0006_nemotron_3_5_benchmark.code.constants import DOMAIN_VOCAB
...
from tasks.t0006_nemotron_3_5_benchmark.code.paths import (
    ANALYSIS_OUTPUT,
    METRICS_JSON,
    NEMOTRON_BATCH_TRANSCRIPTS,
    NEMOTRON_BOOSTED_TRANSCRIPTS,
    RESULTS_DIR,
...
def bca_ci(
    ...
    *,
    ...
    """BCa bootstrap 95% CI with percentile fallback on NaN."""
    try:
        result = bootstrap(
            (per_clip_scores,),
            statistic=np.mean,
    ...
    *,
    ...
    for clip in clips:
        if len(clip.entity_spans) == 0:
            ...
            continue
        hyp_words = set(
            normalise(str(transcripts.get(clip.clip_id, {}).get("hypothesis", ""))).split()
        )
        hits = dels = subs = 0
...
def main() -> None:
    ...
    for path, name in [
        (NEMOTRON_BATCH_TRANSCRIPTS, "nemotron batch"),
        (NEMOTRON_BOOSTED_TRANSCRIPTS, "nemotron word-boosted"),
    ]:
        if not path.exists():
            raise RuntimeError(f"{name} transcripts not found at {path}. Run inference first.")

    batch_transcripts = load_transcripts(NEMOTRON_BATCH_TRANSCRIPTS)
    boosted_transcripts = load_transcripts(NEMOTRON_BOOSTED_TRANSCRIPTS)

    # Whisper baseline from t0004 for comparison (optional — skip if not present)
    ...
    if T0004_WHISPER_BIASED_TRANSCRIPTS.exists():
        ...
        warnings.warn(
            f"t0004 Whisper baseline not found at {T0004_WHISPER_BIASED_TRANSCRIPTS}. "
    ...
    systems.append(
        compute_system_metrics(
            variant_id="nemotron-3.5-batch",
            label="Nemotron 3.5 ASR — batch (no biasing)",
            clips=clips,
    ...
    systems.append(
        compute_system_metrics(
            variant_id="nemotron-3.5-word-boosted",
            label="Nemotron 3.5 ASR — streaming + word boosting",
            clips=clips,
            transcripts=boosted_transcripts,
            anomaly_clips=anomaly_clips,
        )
    )
    ...
    for sys in systems:
        if sys.wer > 0.6:
            raise RuntimeError(
                f"Rejection: {sys.label} WER={sys.wer:.3f} > 0.6. Check transcription."
            )
    ...
    # Streaming degradation deltas
    batch = next(s for s in systems if s.variant_id == "nemotron-3.5-batch")
    boosted = next(s for s in systems if s.variant_id == "nemotron-3.5-word-boosted")
    streaming_degradation = {
        "delta_wer": round(boosted.wer - batch.wer, 6),
        "delta_entity_accuracy": round(boosted.entity_accuracy - batch.entity_accuracy, 6),
        "delta_entity_accuracy_domain_vocab": round(
            boosted.entity_accuracy_domain_vocab - batch.entity_accuracy_domain_vocab, 6
        ),
    }

    metrics_json: dict[str, Any] = {
        "variants": variants,
        "streaming_degradation": streaming_degradation,
    }
    ...
    transcripts_map = {
        "nemotron-3.5-batch": batch_transcripts,
        "nemotron-3.5-word-boosted": boosted_transcripts,
    }
    if whisper_transcripts is not None:
    ...
    print(
        f"{'Variant':<45} {'EA':>7} {'EA_DV':>7} {'WER':>7} {'AC-WER':>7} {'IP':>7} {'Lat p50':>8}"
    )
    print("-" * 95)
    print(
        f"{'Whisper Large v3 + initial_prompt (t0004)':<45} "
    ...
    for sys in systems:
        ...
        print(
            ...
            f"{sys.intent_preservation:>7.4f} {sys.latency['p50']:>7.3f}s"
        )
    print("\nStreaming degradation (boosted vs batch):")
    print(f"  ΔWER:              {streaming_degradation['delta_wer']:+.4f}")
    print(f"  ΔEntity accuracy:  {streaming_degradation['delta_entity_accuracy']:+.4f}")
    print(
        f"  ΔDomain vocab EA:  {streaming_degradation['delta_entity_accuracy_domain_vocab']:+.4f}"
    )
