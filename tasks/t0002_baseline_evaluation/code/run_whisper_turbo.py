"""Run Whisper turbo on all 93 gold-92 clips via faster-whisper (local CPU).

Saves outputs to data/whisper_turbo_transcripts.json.
...
from tasks.t0002_baseline_evaluation.code.paths import WHISPER_TURBO_TRANSCRIPTS
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Run Whisper turbo on gold-92")
    parser.add_argument(
        "--limit",
        type=int,
        default=None,
...
def transcribe_clip(
    *,
    ...
    """Transcribe a single audio clip with Whisper turbo.
...
def main() -> None:
    """Run Whisper turbo inference on gold-92 clips."""
    args = parse_args()
    ...
    for i, clip in enumerate(tqdm(process_clips, desc="Whisper turbo inference")):
        ...
        try:
            ...
            if (i + 1) % 10 == 0:
                print(f"Whisper turbo: {i + 1}/{len(process_clips)} clips processed")
    ...
    # Save results
    WHISPER_TURBO_TRANSCRIPTS.parent.mkdir(parents=True, exist_ok=True)
    with WHISPER_TURBO_TRANSCRIPTS.open("w", encoding="utf-8") as fh:
        json.dump(results, fh, indent=2, ensure_ascii=False)

    print(f"\nDone: {success_count}/{total} clips transcribed successfully.")
    print(f"Saved to {WHISPER_TURBO_TRANSCRIPTS}")
    if failed_count > 0:
