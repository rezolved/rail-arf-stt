"""Run IBM Granite Speech 4.1 2B with keyword biasing on all 93 gold-92 clips.

Biasing mechanism: prompt-based keyword injection.
Prompt: "transcribe the speech to text. Keywords: <kw1>, <kw2>, ..."
Saves outputs to data/granite_biased_transcripts.json.
"""
...
from tasks.t0007_ibm_granite_4_1_benchmark.code.paths import GRANITE_BIASED_TRANSCRIPTS
...
def main() -> None:
    ...
    for i, clip in enumerate(available):
        ...
        hypothesis = transcribe_clip(model, processor, wav, biased_prompt, device)
    ...
    GRANITE_BIASED_TRANSCRIPTS.parent.mkdir(parents=True, exist_ok=True)
    with GRANITE_BIASED_TRANSCRIPTS.open("w", encoding="utf-8") as fh:
        json.dump(results, fh, indent=2, ensure_ascii=False)

    print(f"\nDone: {success}/{total} clips transcribed")
    print(f"Saved to {GRANITE_BIASED_TRANSCRIPTS}")
