...
from tasks.t0011_streaming_stt_benchmark.code.paths import PARAKEET_STREAMING_TRANSCRIPTS
...
def simulate_streaming(audio_float32: np.ndarray) -> tuple[np.ndarray, float]:
    ...
    Returns (accumulated_audio_float32, latency_seconds).
    Latency clock starts at first chunk delivery.
    """
    # Convert float32 → int16 PCM bytes (production WebSocket format)
    pcm_int16 = (audio_float32 * 32767).clip(-32768, 32767).astype(np.int16)
    raw_bytes = pcm_int16.tobytes()

    # Deliver chunks, measure from first chunk
    ...
    # None sentinel → reconstruct and return
    all_bytes = b"".join(accumulated)
    pcm_reconstructed = np.frombuffer(all_bytes, dtype=np.int16).astype(np.float32) / 32767.0

    # Latency will be measured from t_first_chunk to after transcription in caller
